...
const Navbar = ({ onSpeechAssistanceChange }) => {
  ...
  const getWeather = async () => {
    try {
      const response = await fetch(
        process.env.REACT_APP_BACKEND_URL + "/api/weather",
        {
          method: "GET",
          mode: "cors",
          headers: {
      ...
      if (weatherInfo.temperature_fahrenheit) {
        setWeather(
          `${weatherInfo.temperature_fahrenheit}°F - ${weatherInfo.description}`
        );
      } else {
  ...
  return (
    <nav className="flex gap-10 items-center px-5 bg-white border-b border-gray-200 h-[80px]">
      <div className="flex-1 text-xl font-semibold text-gray-700 ml-4">
      ...
      <div className="border-2 border-black p-2 rounded flex flex-row gap-4 justify-center mr-[310px]">
        <span className="text-center py-2 align-middle font-bold">
          Speech Assistance:
        </span>
        <button
          onClick={stopSpeechAssistance}
          className="p-2 text-medium font-medium text-white hover:bg-red-900 rounded-lg transition-colors bg-red-700"
          disabled={isButtonDisabled}
        >
          STOP
        </button>
        <button
          onClick={handleSpeechAssistance}
          className="p-2 text-medium font-medium text-gray-900 hover:bg-green-500 rounded-lg transition-colors bg-green-400"
          disabled={isButtonDisabled}
        >
          ON / OFF
        </button>
      ...
      <button
        className="px-4 py-1 h-14 bg-gray-700 hover:bg-gray-900 text-white font-bold rounded"
